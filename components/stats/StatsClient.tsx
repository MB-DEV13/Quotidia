...
function ContributionGrid({ days }: { days: DayData[] }) {
  const t = useTranslations("stats.client");
  ...
  return (
    <div className="bg-white rounded-2xl shadow-soft p-4">
      <h3 className="text-sm font-semibold text-textDark mb-3">{t("calendarTitle")}</h3>
...
export function StatsClient() {
  ...
  const fetchStats = useCallback(async (p: Period) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) { setError(json.error ?? t("errorLoading")); return; }
    ...
    <div className="space-y-6">
      ...
      {loading ? <StatsSkeleton /> : error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <p className="text-xs text-textLight mb-1">{t("kpiCompletions")}</p>
              <p className="text-2xl font-bold text-primary">{data.summary.totalCompleted}</p>
              <p className="text-xs text-textLight mt-0.5">{t("kpiOnDays", { period })}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <p className="text-xs text-textLight mb-1">{t("kpiAvgRate")}</p>
              ...
              <div className="flex items-center gap-1 mt-0.5">
                {data.trend !== 0 && (
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                    ...
                    {data.trend > 0 ? "▲" : "▼"} {Math.abs(data.trend)}% {t("trendVsStart")}
                  </span>
                )}
              </div>
            </div>
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <p className="text-xs text-textLight mb-1">{t("kpiExpenses")}</p>
              <p className="text-2xl font-bold text-danger">{formatCurrency(data.summary.totalExpenses)}</p>
              {data.totalIncome > 0 && (
                <p className="text-[10px] text-success mt-0.5">{t("revenues", { amount: formatCurrency(data.totalIncome) })}</p>
              )}
            </div>
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <p className="text-xs text-textLight mb-1">{t("kpiActiveHabits")}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl shadow-soft p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-textLight">{t("trendLabel")}</p>
                <p className="font-bold text-textDark">
                  {data.trend > 0 ? t("trendImproving") : data.trend < 0 ? t("trendDeclining") : t("trendStable")}
                </p>
            ...
            {data.bestDayOfWeek && (
              <div className="bg-white rounded-2xl shadow-soft p-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs text-textLight">{t("bestDayLabel")}</p>
                  <p className="font-bold text-textDark">{data.bestDayOfWeek.label}</p>
                  <p className="text-xs text-textLight mt-0.5">{t("bestDayRate", { rate: data.bestDayOfWeek.rate })}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-soft p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-textDark">{t("chartCompletionTitle", { period })}</h3>
              <span className="text-xs bg-primary/10 text-primary font-semibold px-2 py-1 rounded-lg">
                {t("chartAvgLabel", { rate: data.avgCompletionRate })}
              </span>
            </div>
            {data.days.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={data.days}>
                  ...
                  <Tooltip formatter={(value: number) => [`${value}%`, t("tooltipRate")]}
              ...
              <p className="text-sm text-textLight text-center py-8">{t("noData")}</p>
            )}
          </div>
          ...
          {data.topHabits.length > 0 && (
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <h3 className="text-sm font-semibold text-textDark mb-4">{t("topHabitsTitle")}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {data.expensesByCategory.length > 0 && (
              <div className="bg-white rounded-2xl shadow-soft p-4">
                <h3 className="text-sm font-semibold text-textDark mb-4">{t("categoryTitle")}</h3>
            ...
            {data.expensesChart.length > 0 && (
              <div className="bg-white rounded-2xl shadow-soft p-4">
                <h3 className="text-sm font-semibold text-textDark mb-4">{t("monthlyTitle")}</h3>
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={data.expensesChart} barSize={28}>
                    ...
                    <Tooltip formatter={(value: number) => [formatCurrency(value), t("tooltipTotal")]}
                      contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 16px rgba(0,0,0,0.1)" }} />
          ...
          {(data.summary.totalExpenses > 0 || data.totalIncome > 0) && (
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <h3 className="text-sm font-semibold text-textDark mb-4">{t("incomeVsExpTitle", { period })}</h3>
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-success font-medium">{t("income")}</span>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-danger font-medium">{t("expenses")}</span>
                ...
                <div className="flex justify-between items-center pt-1 border-t border-gray-100">
                  <span className="text-xs text-textLight">{t("netBalance")}</span>
          ...
          {data.goalsStats.total > 0 && (
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <h3 className="text-sm font-semibold text-textDark mb-4">{t("goalsTitle")}</h3>
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="text-center">
                  <p className="text-xl font-bold text-primary">{data.goalsStats.active}</p>
                  <p className="text-[10px] text-textLight mt-0.5">{t("goalsActive")}</p>
                </div>
                <div className="text-center border-x border-gray-100">
                  <p className="text-xl font-bold text-success">{data.goalsStats.completed}</p>
                  <p className="text-[10px] text-textLight mt-0.5">{t("goalsCompleted")}</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-accent">{data.goalsStats.globalPct}%</p>
                  <p className="text-[10px] text-textLight mt-0.5">{t("goalsProgress")}</p>
                </div>
              </div>
              {data.goalsStats.bestGoal && (
                <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
                  ...
                  <span className="text-[10px] text-textLight shrink-0">{t("goalsMostAdvanced")}</span>
                </div>
              )}
            </div>
          )}
